...
His cows don't like this barn layout and become aggressive towards each other once put into a stall.
 To prevent the cows from hurting each other, John wants to assign the cows to the stalls, such that the minimum
...
class AggressiveCows {
    static boolean isCowsPlaced(int A[], int B, int dist) {
        int cows = 1;
        int prevCow = A[0];
    ...
    static int solve(int A[], int B) {
        int n = A.length;
        int count = 0;
        int left = 0, right = A[n - 1] - A[0], mid;
        while (left <= right) {
